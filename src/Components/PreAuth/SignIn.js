    ...
    <div className='lowersectionland'>
        <div style={{height:width>700?"275px":"300px",width:width>700?"40%":"95%",boxSizing:"border-box",border:width>700?"2.5px solid white":'',borderRadius:"10px",backgroundColor:'#0d47a1'}}>
            ...
            <div style={{height:'80%',display:'flex',flexDirection:'column',justifyContent:"space-between"}}>
              <div style={{height:"27%",width:'100%',margin:'0px auto',border:'1px solid white',borderWidth:'0px 0px 1px',display:'flex',justifyContent:'center',alignItems:'flex-end',position:'relative'}}><p style={{position:'absolute',color:'white',fontSize:'18px',fontFamily:'NexaTextLight',top:'5px',left:'5px'}}>Enter Email:</p><input onChange={(event)=> changeFieldData(event)} name="Email" value={formObject.Email} style={{width:'100%',fontSize:'25px',color:'white',height:'60%',backgroundColor:'transparent',borderWidth:'0px',paddingLeft:"15px",outline:'none'}} type="email"/></div>
              ...
              <div style={{height:"27%",display:"flex",justifyContent:'center',alignItems:"center"}}>{ loading=== false?<input type='button'  onClick={(event)=>submitreg(event)} value='Sign in' style={{width:width>700?'80px':"150px",height:width>700?'30px':"50px",borderRadius:"15px",borderWidth:'0px'}}/>:<i class="fa fa-spinner fa-spin" style={{fontSize:'20px',color:'white'}}></i>}</div>
            </div>
        </div>
    </div>
