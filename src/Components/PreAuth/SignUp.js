  ...
  const navigate = useNavigate()
    ...
    const submitreg = async (event)=>{
        ...
        if(formObject.Username==='' || formObject.Password==='' || formObject.Email===''){
          ...
          setErrObj({status:'error',message:'fill all fields'})
...
window.localStorage.setItem('Userdata',JSON.stringify(payloadData))
 ...
 navigate('/landingpage')
  ...
  return (
    <div style={{position:"relative",zIndex:"2",backgroundColor:'#0d47a1',padding:showError?'0px':width>700?'50px':'15px',color:'white',boxSizing:"border-box",width:"100vw",height:"100vh",margin:'0px auto'}}>
    ...
    <div style={{height:width>700?"330px":"350px",width:width>700?"40%":"95%",boxSizing:"border-box",border:width>700?"2.5px solid white":'',borderRadius:"10px",backgroundColor:'#0d47a1'}}>
            ...
            <div style={{ height:width>700?"25%":"20%",width:'100%',margin:'0px auto',border:'1px solid white',borderWidth:'0px 0px 1px',display:'flex',justifyContent:'center',alignItems:'flex-end',position:'relative'}}><p style={{position:'absolute',color:'white',fontSize:width>700?"18px":'13px',fontFamily:'NexaTextLight',top:'5px',left:'5px'}}>Enter Username:</p><input onChange={(event)=> changeFieldData(event)} name='Username' value={formObject.Username} style={{width:'100%',fontSize:'20px',fontFamily:"NexaTextLight",color:'white',height:'60%',backgroundColor:'transparent',borderWidth:'0px',paddingLeft:"15px",outline:'none'}} type="text"/></div>
              ...
              <div style={{ height:width>700?"25%":"20%",display:"flex",justifyContent:'center',alignItems:"center"}}>{ loading=== false?<input type='button'  onClick={(event)=>submitreg(event)} value='Sign Up' style={{width:width>700?'80px':"150px",height:width>700?'30px':"50px",borderRadius:"15px",borderWidth:'0px'}}/>:<i class="fa fa-spinner fa-spin" style={{fontSize:'20px',color:'white'}}></i>}</div>
            </div>
        </div>
